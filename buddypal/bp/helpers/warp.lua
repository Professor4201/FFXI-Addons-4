--------------------------------------------------------------------------------
-- Warp helper: Handles all Warp functions.
--------------------------------------------------------------------------------
local warp = {}
function warp.new()
    local self = {}
        ...
        for _,v in pairs(npcs) do
            ...
            if type(v) == "table" and (v.name):match(name) then
                ...
                if count == 1 and null ~= 0 then
                    table.insert(list, {name=string.format("** %+25s: %s [%04s] **\n", v.name, v.id, v.index), y=position.y, position={x=v.x, y=v.y, z=v.z}, r=text.r, g=text.g, b=text.b})
                    count = (count + 1)
                    
                elseif null ~= 0 then
                    table.insert(list, {name=string.format("** %+25s: %s [%04s] **\n", v.name, v.id, v.index), y=(list[count-1].y+margin), position={x=v.x, y=v.y, z=v.z}, r=text.r, g=text.g, b=text.b})
                    count = (count + 1)
        ...
        for _,v in pairs(warps["homepoints"]) do
            ...
            if type(v) == "table" and (v.name):match(name) then
                ...
                if count == 1 and (null ~= 0 and null ~= -0.19999992847443) then
                    table.insert(list, {name=string.format("** %+25s: %s [%04s] **\n", v.name, v.id, v.index), y=position.y, position={x=v.x, y=v.y, z=v.z}, r=53, g=235, b=229})
                    count = (count + 1)
                    
                elseif (null ~= 0 and null ~= -0.19999992847443) then
                    table.insert(list, {name=string.format("** %+25s: %s [%04s] **\n", v.name, v.id, v.index), y=(list[count-1].y+margin), position={x=v.x, y=v.y, z=v.z}, r=53, g=235, b=229})
                    count = (count + 1)
