--------------------------------------------------------------------------------
-- DRK Core: Handle all job automation for Dark Knight.
--------------------------------------------------------------------------------
local core = {}
            ...
            if player.status == 1 then
                ...
                if settings["WS"]:current() and bpcore:canAct() then
                    ...
                    if settings["AM"]:current() then
                        ...
                        if bpcore:buffActive(272) and player["vitals"].tp > 1000 then
                            ...
                            if settings["SANGUINE"]:current() and player["vitals"].hpp > system["DRK"]["Sanguine Threshold"] then
                ...
                if settings["JA"]:current() and bpcore:canAct() then
                    ...
                    if player.sub_job == "RDM" then
                        ...
                        -- CONVERT LOGIC.
                        if settings["CONVERT"]:current() and player["vitals"].hpp > system["DRK"]["Convert Threshold"].hpp and player["vitals"].mpp < system["DRK"]["Convert Threshold"].mpp then
                ...
                if settings["HATE"]:current() then
                    ...
                    elseif player.sub_job == "DRK" then
                        ...
                        if bpcore:canAct() and (os.clock()-system["DRK"]["Hate Timer"]) > system["DRK"]["Hate Delay"] then
                            ...
                            if bpcore:isJAReady(JA["Last Resort"].recast_id) and bpcore:getAvailable("JA", "Last Resort") then
                                ...
                                system["DRK"]["Hate Timer"] = os.clock()
                    ...
                    elseif player.sub_job == "BLU" and bpcore:canCast() then
                        ...
                        if settings["AOEHATE"]:current() and (os.clock()-system["DRK"]["Hate Timer"]) > system["DRK"]["Hate Delay"] then
                ...
                if settings["DEBUFFS"]:current() then
                    ...
                    if player.sub_job == "DNC" and bpcore:canAct() then
                        ...
                        -- STEPS.
                        if bpcore:isJAReady(JA[settings["STEPS"]:current()].recast_id) and os.clock()-system["DRK"]["Steps Timer"] > system["DRK"]["Steps Delay"] and bpcore:getAvailable("JA", settings["STEPS"]:current()) then
