...
function core.get()
    ...
    self.handleAutomation = function()
        ...
        if bpcore:checkReady() and not helpers["actions"].getMoving() and system["BP Enabled"]:current() then
            ...
            if system["Player"].status == 1 then
                ...
                if settings["WS"]:current() and bpcore:canAct() then
                    ...
                    if settings["AM"]:current() then
                        ...
                        elseif not bpcore:buffActive(272) and system["Player"]["vitals"].tp == settings["AM THRESHOLD"] and system["Weapon"].en == "Apocalypse" and bpcore:getAvailable("WS", "Catastrophe") then
                        ...
                        elseif not bpcore:buffActive(272) and system["Player"]["vitals"].tp == settings["AM THRESHOLD"] and system["Weapon"].en == "Redemption" and bpcore:getAvailable("WS", "Quietus") then
                        ...
                        elseif not bpcore:buffActive(272) and system["Player"]["vitals"].tp == settings["AM THRESHOLD"] and system["Weapon"].en == "Liberator" and bpcore:getAvailable("WS", "Insurgency") then
                            helpers['queue'].addToFront(WS["Namas Arrow"], "t")
                            
                        elseif not bpcore:buffActive(272) and system["Player"]["vitals"].tp == settings["AM THRESHOLD"] and system["Weapon"].en == "Anguta" and bpcore:getAvailable("WS", "Entropy") then
                            helpers['queue'].addToFront(WS["Namas Arrow"], "t")
                    ...
                    elseif system["Player"]["vitals"].tp > settings["TP THRESHOLD"] then
                        ...
                        if bpcore:getAvailable("WS", settings["WSNAME"]) then
                            helpers['queue'].addToFront(WS[settings["WSNAME"]], "t")                            
                        end
            ...
            elseif system["Player"].status == 0 or ( settings["SUPER-TANK"]:current() and system["Player"].status ~= 2 and system["Player"].status ~= 3 and (helpers["target"].getTarget() or windower.ffxi.get_mob_by_target("t")) ) then
                ...
                if settings["HATE"]:current() then
                    ...
                    -- PROVOKE.
                    if bpcore:isJAReady(JA["Provoke"].recast_id) and bpcore:getAvailable("JA", "Provoke") and bpcore:canEngage(target) then
                    ...
                    if system["Player"].sub_job == "DNC" then
                        ...
                        -- ANIMATED FLOURISH.
                        if bpcore:isJAReady(JA["Animated Flourish"].recast_id) and bpcore:getFinishingMoves() > 0 and bpcore:getAvailable("JA", "Animated Flourish") and bpcore:canEngage(target) then
