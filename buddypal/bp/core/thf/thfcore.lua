--------------------------------------------------------------------------------
-- THF Core: Handle all job automation for Thief.
--------------------------------------------------------------------------------
local core = {}
            ...
            if player.status == 1 then
                ...
                if settings["WS"]:current() and bpcore:canAct() then
                    ...
                    if settings["AM"]:current() then
                        ...
                        if bpcore:buffActive(272) and player["vitals"].tp > 1000 then
                            ...
                            if settings["SANGUINE"]:current() and player["vitals"].hpp > system["THF"]["Sanguine Threshold"] then
                ...
                if settings["JA"]:current() and bpcore:canAct() then
                    ...
                    if player.sub_job == "RDM" then
                        ...
                        -- CONVERT LOGIC.
                        if settings["CONVERT"]:current() and player["vitals"].hpp > system["THF"]["Convert Threshold"].hpp and player["vitals"].mpp < system["THF"]["Convert Threshold"].mpp then
                ...
                if settings["HATE"]:current() then
                    ...
                    elseif player.sub_job == "DRK" then
                        ...
                        if bpcore:canAct() and (os.clock()-system["THF"]["Hate Timer"]) > system["THF"]["Hate Delay"] then
                    ...
                    elseif player.sub_job == "BLU" and bpcore:canCast() then
                        ...
                        if settings["AOEHATE"]:current() and (os.clock()-system["THF"]["Hate Timer"]) > system["THF"]["Hate Delay"] then
                ...
                if settings["DEBUFFS"]:current() then
                    ...
                    if player.sub_job == "DNC" and bpcore:canAct() then
                        ...
                        -- STEPS.
                        if bpcore:isJAReady(JA[settings["STEPS"]:current()].recast_id) and os.clock()-system["THF"]["Steps Timer"] > system["THF"]["Steps Delay"] and bpcore:getAvailable("JA", settings["STEPS"]:current()) then
