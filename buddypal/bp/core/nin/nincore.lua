--------------------------------------------------------------------------------
-- NIN Core: Handle all job automation for Ninja.
--------------------------------------------------------------------------------
local core = {}
...
function core.get()
    ...
    settings["SPELLS"]={
        ...
        ["Dia"] = {["allowed"]=0,["delay"]=60},
        ["Dia II"] ={["allowed"]=0,["delay"]=120},
        ["Bio"] = {["allowed"]=0,["delay"]=60},
        ["Bio II"] ={["allowed"]=0,["delay"]=120},
        ["Distract"] = {["allowed"]=0,["delay"]=120},
        ["Distract II"] ={["allowed"]=0,["delay"]=120},
            ...
            if player.status == 1 then
                ...
                if settings["WS"]:current() and bpcore:canAct() then
                    ...
                    if settings["AM"]:current() then
                        ...
                        if bpcore:buffActive(272) and player["vitals"].tp > 1000 then
                            ...
                            if settings["SANGUINE"]:current() and player["vitals"].hpp > system["NIN"]["Sanguine Threshold"] then
                ...
                if settings["JA"]:current() and bpcore:canAct() then
                    ...
                    if player.sub_job == "RDM" then
                        ...
                        -- CONVERT LOGIC.
                        if settings["CONVERT"]:current() and player["vitals"].hpp > system["NIN"]["Convert Threshold"].hpp and player["vitals"].mpp < system["NIN"]["Convert Threshold"].mpp then
                ...
                if settings["HATE"]:current() then
                    ...
                    elseif player.sub_job == "DRK" then
                        ...
                        if bpcore:canAct() and (os.clock()-system["NIN"]["Hate Timer"]) > system["NIN"]["Hate Delay"] then
                            ...
                            if bpcore:isJAReady(JA["Last Resort"].recast_id) and bpcore:getAvailable("JA", "Last Resort") then
                                ...
                                system["NIN"]["Hate Timer"] = os.clock()
                    ...
                    elseif player.sub_job == "BLU" and bpcore:canCast() then
                        ...
                        if settings["AOEHATE"]:current() and (os.clock()-system["NIN"]["Hate Timer"]) > system["NIN"]["Hate Delay"] then
                ...
                if settings["DEBUFFS"]:current() then
                    ...
                    if player.sub_job == "DNC" and bpcore:canAct() then
                        ...
                        -- STEPS.
                        if bpcore:isJAReady(JA[settings["STEPS"]:current()].recast_id) and os.clock()-system["NIN"]["Steps Timer"] > system["NIN"]["Steps Delay"] and bpcore:getAvailable("JA", settings["STEPS"]:current()) then
