...
function core.get()
    ...
    self.handleCommands = function(commands)
        ...
        elseif command == "weather" then
            local weather = windower.convert_auto_trans(commands[2]):sub(1,3):lower() or false
            if weather then

                for _,v in pairs(settings["WEATHER"]) do

                    if v and type(v) == 'string' and weather == v:sub(1,3):lower() then
                        settings["WEATHER"]:setTo(v)
                        helpers['popchat']:pop(("Auto-Weather now set to: " .. tostring(settings["WEATHER"]:current())):upper(), system["Popchat Window"])    
                    end
