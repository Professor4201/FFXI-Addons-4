...
-- Launch Events
local geo = {}
geo["zone change"] = function(new, old)
    helpers["events"].setZone()
    helpers["events"].zoneUpdate()
    
end

geo['prerender'] = function()
    local player = windower.ffxi.get_mob_by_target("me") or false
    
    if player then
        
        if helpers["events"].getReset() then
            settings    = dofile(windower.addon_path.."bp/events/sequences/jobs/settings/geo_settings.lua")
...
geo["Handle Messages"] = function(id,original,modified,injected,blocked)
