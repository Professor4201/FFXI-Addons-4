local settings = require('bp/events/sequences/couriers/settings/legend_settings')
        ...
        if os.clock()-clocks.ping > delays.spam + delays.zone and windower.ffxi.get_mob_by_target('me').status ~= 4 and not midaction then
            ...
            elseif zone.name == "Western Adoulin" then
                ...
                elseif status == 1 then
                    ...
                    if not locked then
                        windower.send_command("bp mw wp Kamihr Drifts 0")
                        helpers["events"].setDelays("zone", 10)
                ...
                elseif status == 3 then
                    ...
                    if not locked then
                        ...
                        if imps >= 3 and helpers["coalitions"].getRepeating() then
                            helpers["events"].finishEvent("Couriers", helpers["coalitions"].getCoalition(), "Legend")
                            
                        else
                            helpers["events"].finishEvent("Couriers")
            ...
            elseif zone.name == "Kamihr Drifts" then
                ...
                if status == 1 then
                    helpers['events'].interact(npc[zone.name]["Station Administrator"], 1, 2)
                    
                elseif status == 2 then
                    windower.send_command("bp mw wp Western Adoulin 1")
                    helpers["events"].setDelays("zone", 10)
